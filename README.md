...
本项目用于在 Windows 上批量处理论文 PDF：每篇论文都在已登录的 ChatGPT 网页版中新建一个独立对话，只上传这一篇 PDF，并发送统一的分析提示词；回答完成后，扩展自动在网页中选中整条 GPT 回答，再调用本机 Microsoft Word，以“保留源格式”粘贴并保存分析结果。

当前 Chrome 扩展版本：`1.2.2`。
...
7. 等待 ChatGPT 回答结束。
8. 在网页中选中当前 GPT 回答的全部正文，页面会显示蓝色选区。
9. 通过 Chrome 可信键盘事件发送 `Ctrl+C`。
10. 校验剪贴板内容确实属于当前回答；不匹配时最多自动重试三次。
11. 本地服务启动 Microsoft Word，执行“保留源格式”粘贴。
12. Word 保存后，再次检查文档正文是否与当前 ChatGPT 回答一致。
13. 验证通过后保存论文名 `.docx`，并把任务设为 `completed`。
14. 单篇失败时记录原因并继续下一篇，同一批次不会立即重复失败任务。
...
本项目按你的 ChatGPT Business 网页账号设计。运行前请先在 Chrome 中手动登录，并确认可以正常新建对话、上传一个 PDF 和发送消息。
...
- `debugger`：聚焦受控标签页并发送浏览器认可的可信 `Ctrl+C` 键盘事件；
...
使用可信复制时，Chrome 可能短暂显示“扩展正在调试此浏览器”之类提示，这是 `debugger` 权限的正常表现。
...
点击“打开 output”，会通过本地服务在 Windows 资源管理器中直接打开项目的 `output` 文件夹。本地服务必须正在运行，扩展令牌也必须正确。
...
插件提供独立的“把当前回答归档到所选任务 Word”按钮。它使用与自动任务相同的“蓝底全选回答 → 可信 `Ctrl+C` → Word 保留源格式粘贴 → 正文校验”流程，并把结果正式归档到论文任务。

使用方法：

1. 先停止自动任务队列；
2. 在 Chrome 中打开包含目标回答的 ChatGPT 对话；
3. 等待回答生成完成；
4. 在任务列表中只勾选与该对话对应的一篇论文；
5. 点击“把当前回答归档到所选任务 Word”；
6. 扩展会选中当前页面最后一条 GPT 回答，页面短暂显示蓝色选区；
7. 扩展发送可信 `Ctrl+C` 并校验剪贴板；
8. 本地服务生成论文名 `.docx`，保存到该任务的 `output` 目录；
9. 系统保存当前对话 URL，并根据 Word 检查结果把状态设为 `completed` 或 `needs_review`。

按钮要求只选择一个任务。自动队列正在运行时会拒绝手动归档，以免两个流程同时修改剪贴板或任务状态。自动执行论文任务时也会直接使用同一套整条回答选择复制逻辑，无需人工点击此按钮。
...
6. 最长等待约 45 分钟；
7. 等待回答停止且文本稳定；
8. 精确定位当前回答的 Markdown 正文容器；
9. 用 Selection/Range 选中全部回答，使页面显示蓝色选区；
10. 通过可信键盘事件发送 `Ctrl+C`；
11. 读取并核对剪贴板；
12. 启动 Word 原格式粘贴；
13. 验证 Word 正文；
14. 保存对话 URL 和任务状态；
...
```text
网页选中整条 GPT 回答 → Ctrl+C → 系统剪贴板 → Microsoft Word“保留源格式”粘贴 → .docx
```
...
- 网页没有完整选中当前 GPT 回答，或 `Ctrl+C` 没有正确更新剪贴板；
- Word 粘贴内容与当前回答不一致；
- Word 文件被打开并锁定；
- 回答太短；
- Word 文件小于 20 KB；
...
当前版本会先把整条 GPT 回答选成蓝底，再自动激活 ChatGPT 标签页并发送 `Ctrl+C`；启动 Word 前会校验剪贴板，Word 保存后还会校验正文。如果仍发生，确认扩展版本是 `1.2.2` 并重新加载扩展。不要在任务完成瞬间手动复制其他内容。

### 16.9 只复制了表格或代码

扩展不会点击回答内部的“复制表格”或“复制代码”，而是选中最后一条 GPT 回答的整个正文容器。如果仍只复制局部内容，说明 ChatGPT 更新了页面结构，可能需要更新 `content.js` 的回答正文选择器。
...
这是发送可信 `Ctrl+C` 所需的正常提示。扩展只在复制当前选区时短暂连接 Chrome 调试协议，随后立即断开。
